...
export default function ScheduleSidebar({ onGenerate }: ScheduleSidebarProps) {
  const { selectedCourses, removeCourse, clearSelectedSections, clearCourses } = useScheduleStore();
  ...
  const totalCombinations = selectedCourses.reduce((product, c) => {
    const count = c.selectedSections.length > 0 ? c.selectedSections.length : c.sections.length;
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-xl">
        ...
        {selectedCourses.length === 0 ? (
          ...
          selectedCourses.map((course, idx) => (
            <SelectedCourseItem
              ...
              onClearSelection={() => clearSelectedSections(course.subject, course.course_number)}
            />
          ))
        )}
      </div>
      ...
      {selectedCourses.length > 0 && (
        <div className="px-4 py-3.5 border-t border-gray-200 space-y-2.5">
          ...
          {onGenerate && (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Sparkles size={16} />
              Generate Schedules
            </button>
